...
export default function App() {
  ...
  const [theme,    setTheme]    = useState(
    () => localStorage.getItem("rs_theme") || "dark"
  );
  const [data, setData] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("repsense_data") || "{}");
    } catch {
      return {};
    }
  });
  ...
  function saveWorkout(reps, ex, duration, formScore) {
    if (!reps) return;
    const updated = {
